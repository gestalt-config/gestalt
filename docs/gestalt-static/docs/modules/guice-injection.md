...
The Guice module allows Gestalt to inject configuration directly into your classes using Guice and the `@InjectConfig` annotation on any class fields. This does not support constructor injection due to Guice limitations.
To enable this, add the `new GestaltModule(gestalt)` to your Guice Modules, then pass in your instance of Gestalt.
